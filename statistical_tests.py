...
def bartlett_test(data1,data2,alpha,flag):
    ...
    for col in range(cols):
        ...
        if (P_value * 100) > alpha:
            print(
                'Com um p de {:.4f}, há evidências suficientes para confirmar a hipótese de que os dados para [{}] possuem a variâncias iguais'.format(
                    P_value, data_columns[col]))
        else:
            print(
                'Com um p de {:.4f}, há evidências suficientes para negar a hipótese de que os dados para [{}] possuem a variâncias iguais'.format(
                    P_value, data_columns[col]))
